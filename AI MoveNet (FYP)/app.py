...
def isBoxing(curr_body_points):
    # 定义左手臂和右手臂的直线组合
    left_boxing_line = [11, 13, 15, 17, 19, 21]  # 左手臂
    right_boxing_line = [12, 14, 16, 18, 20, 22]  # 右手臂
    
    # 检查右手臂是否在一条直线上
    if isStraightLine(curr_body_points, right_boxing_line):
        # 如果右手臂是一条直线，检查左手臂的位置是否符合要求
        if (curr_body_points[13][2] > curr_body_points[11][2] and  # 检查左肘是否在左肩下方
            curr_body_points[13][2] > curr_body_points[15][2] and  # 检查左肘是否在左手腕下方
            abs(curr_body_points[11][2] - curr_body_points[23][2]) < abs(curr_body_points[11][2] - curr_body_points[15][2])):  # 检查左肘是否在左肩和左臀之间
            return True
    # 如果右手臂不在一条直线上，则检查左手臂是否在一条直线上
    elif isStraightLine(curr_body_points, left_boxing_line):
        if (curr_body_points[14][2] > curr_body_points[12][2] and  # 检查右肘是否在右肩下方
            curr_body_points[14][2] > curr_body_points[16][2] and  # 检查右肘是否在右手腕下方
            abs(curr_body_points[12][2] - curr_body_points[24][2]) < abs(curr_body_points[12][2] - curr_body_points[16][2])):  # 检查右肘是否在右肩和右臀之间
            return True
...
def detect_movement():
    ...
    # Initialize the PoseDetector and other necessary variables
    ...
    results = {
        ...
        "Yoga": False,
        "Tabata": False,
        "NoPersonDetected": False  # 新增：用于标记是否检测到人
    }
    
    # Open the video stream from the URL
    cap = cv2.VideoCapture(video_url)
    if not cap.isOpened():
        return jsonify(results)  # Return empty results if video cannot be opened
    ...
    while True:
        ...
        # Process the frame and detect actions
        img = detector.findPose(img)
        body_points = detector.findPosition(img)
        
        # 新增：检查是否有人，如果没有人，则更新结果并跳出循环
        if not body_points:
            results["NoPersonDetected"] = True
            break

        if prev_body_points is not None:
            results["Abs"] = results["Abs"] or isAbs(body_points)
            results["PushUp"] = results["PushUp"] or isPushUp(prev_body_points, body_points)
            results["Boxing"] = results["Boxing"] or isBoxing(body_points)
            results["HIIT"] = results["HIIT"] or isHIIT(body_points)
            results["Leg"] = results["Leg"] or isLeg(body_points)
            results["Yoga"] = results["Yoga"] or isYoga(body_points)
            results["Tabata"] = results["Tabata"] or isTabata(body_points)

            # If the specified operation has a result, break the loop
            if results.get(operation, False):
                break
    ...
    # Return the action detection results
